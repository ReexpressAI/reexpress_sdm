#########################################################################################################
##################### Getting Started Tutorial for the Command line Interface (CLI) #####################
#########################################################################################################
...
# (The publicly available .jsonl data files at https://github.com/ReexpressAI/sdm_activations/releases/tag/v1.0.0 can also be used as a drop-in replacement for the factcheck_gemma_4_31b_it_4bit data if you want to walk through this tutorial with other models or tasks, such as sentiment classification.)
...
PROJECT_DIR="/Users/a/Documents/project_tutorials"  # choose a suitable directory; we will be using this below to organize the output
...
# Note that --composition determines what fields in the JSON lines files will be used as input to the SDM activation. For convenience, we separate "embedding" and "attributes" if, for bookkeeping, you want to separate the core embeddings from other features, but you are free to choose the semantics of those fields, or to altogether ignore that distinction and only use the "embedding" field, or only the "attributes" field. If you choose "embedding+attributes", both vectors are concatenated together as input to the SDM activation. The important point is to be consistent for a given model and subsequent evaluation/deployment.

# FYI: If specified at training, the same values for --representation_fingerprint and --class_names must be used at test time, including when importing into Reexpress two.
...
echo ${MODEL_DIR}/logs.txt  # training output is redirected to logs.txt; use `tail -f ${MODEL_DIR}/logs.txt` to watch progress
#/Users/a/Documents/project_tutorials/models/factcheck.gemma_4_31b_it_4bit/logs.txt

# `sdm train` auto-selects the device (CUDA, then MPS, then CPU); add `--device=cpu|mps|cuda` to choose explicitly.
...
echo ${MODEL_DIR}/report.txt  # summary report of training
...
# provides summary stats, including the q'_min values and thresholds for each calibration region for which the class- and prediction-conditional accuracy is estimated to be at least a given alpha:
sdm artifact inspect --model ${MODEL_FILE}
# In this case, the most conservative region into which the calibration set was able to be partitioned was alpha=0.97, followed by alpha 0.89 and 16 additional descending regions down to 0.52.
...
# With the "ood_eval" file, the marginal accuracy of the 0.97 region is 0.974 (38 out of 39 documents). The overall accuracy (average over the full dataset, not just a particular region) is much lower at 0.83, which without the SDM estimator, would come as a surprise to users, since the marginal accuracy of the calibration set was 0.946 (as shown in best_calibration_scored.overall_evaluation_report.json, the score report for the calibration set, generated below). After the CLI tutorial, we will examine that 1 missed prediction in the alpha=0.97 region with the Reexpress two app.
...
# the fixed adaptor". In Reexpress two, you can also preview such changes without overwriting the model if
...
# be rerun with this updated model, as in the previous section with the original ${MODEL_FILE}.

# (The remaining sections use the original ${MODEL_FILE}.)
...
# initially training, but the exemplar dimension, class order, input feature dimension, and representation fingerprint must stay fixed. Note that by default the data will continue to be randomly shuffled at the
...
# Note that there is currently no equivalent option for constructing a summary evaluation report for the best iteration training set directly from the CLI, but this can be readily achieved with the Python API:

python - \
  "${MODEL_FILE}" \
  "${EVAL_OUTPUT_DIR}/best_training_scored.sdmdataset" \
  "${EVAL_OUTPUT_DIR}/best_training_scored.overall_evaluation_report.json" <<'PY'
import json
import sys
from pathlib import Path
from reexpress_sdm import Dataset, SDMController

model_path, input_path, output_path = sys.argv[1:]
controller = SDMController.load(model_path)
...
# Require the explicit exclusions saved by selected-training export.
identities = [row["excludedSupportIndex"] for row in dataset.rows]
if any(index is None for index in identities):
    raise ValueError("Every selected-training row must have a support exclusion")

scores = controller.score(
    dataset.vectors,
    ids=dataset.ids,
    representation_fingerprint=dataset.representation_fingerprint,
    identity_support_indices=identities,
)
report = {
    "evaluation": controller.evaluate(scores, dataset.labels),
    "distribution": controller.summarize_distribution(scores),
}
...
# This recomputes scores with self-exclusions and writes the same evaluation and distribution report structure as the CLI. Missing exclusions cause an error.
...
# At the same time, the raw .jsonl files have the advantage of more general portability, as the embedding/attributes are simply saved as a numeric array in JSON.
...
# Alternatively, if you (or your AI agent) have already trained a model as above, you can directly import it into the app for analysis. The app
...
# alpha regions at 0.97, 0.89, 0.88, 0.85, 0.82, 0.8, 0.79, 0.76, 0.75, 0.68, 0.67, 0.66, 0.65, 0.62, 0.59, 0.57, 0.55, 0.52 and their associated Minimum rescaled Similarities and class thresholds. (See https://raw.githubusercontent.com/ReexpressAI/Reexpress_two/main/documentation/tutorials/python_cli_to_app_tutorial/overview_imported_model_but_not_data.jpg)

# Next, navigate to the Data tab. As noted in the section "Scoring the training and calibration sets", attach the
# scored splits for the best iteration training and calibration splits (i.e., best_calibration_scored.sdmdataset and best_training_scored.sdmdataset created from above with `sdm dataset export-sources`). (See https://raw.githubusercontent.com/ReexpressAI/Reexpress_two/main/documentation/tutorials/python_cli_to_app_tutorial/attach_sources.jpg)

# Next, we will also add our already-scored held-out test data. Still in the Data tab, click "Import data". First, select ood_eval.gemma_4_31b_it_4bit.per_document_scores.jsonl in the open file dialog box. Next, for the dataset split, select "Evaluation". (In this case, the file doesn't have an "attributes" field, so keeping the default "Use attributes vectors when present" has no effect. If the file did have an "attributes" field, it would be necessary to uncheck "Use attributes vectors when present" since --composition="embedding" was used during training.) Next, do the same for ood_eval.ood_random_shuffle.gemma_4_31b_it_4bit.per_document_scores.jsonl, again selecting "Evaluation" for the dataset split.

# In the Train tab, we can view the training curves for both iterations. (See https://raw.githubusercontent.com/ReexpressAI/Reexpress_two/main/documentation/tutorials/python_cli_to_app_tutorial/train.jpg) In this view, we can also start training again, or continue training from these weights. And we can always walk back to previous models by activating the weights in the "Model History" section at the bottom of the screen. (Note that when activating a new set of weights, it is necessary to rescore the documents.)

# Next, go to the Analysis tab and select the ood_eval.gemma_4_31b_it_4bit.per_document_scores.jsonl split. Your output may differ from ours (especially if you chose different training parameters), but in our run above, there was 1 point in the 0.97 region whose prediction did not match the ground-truth labels. We can narrow to that subset by clicking "Select documents" in the upper left and choosing "HR subset: Minimum accepted alpha" and then choosing alpha>=0.97. (With SDM estimators, the High-Reliability (HR) region refers to the set of points for which the class- and prediction-conditional accuracy is estimated to be at least the given alpha value, which in this case is 0.97. Since alpha=0.97 is the most conservative region here, we could equivalently have selected "HR subset: Exactly one region" and then chosen alpha=0.97.) (See https://raw.githubusercontent.com/ReexpressAI/Reexpress_two/main/documentation/tutorials/python_cli_to_app_tutorial/ood_eval_alpha97_region.jpg) Also select "Outcome: Incorrect only". Then either in Charts or Documents we can select the incorrect point. After clicking the point in Charts (or selecting the row in Documents), select Report. The document's text is "JUBALAND is a state in South Sudan, located in Central Equatoria." for which the ground-truth label is 1 (which corresponds to "true statement" for this task). This is actually an annotation error; Jubaland is a federal member state of Somalia and not South Sudan. The SDM estimator is relatively confident in this point with a Rescaled Similarity (q') of 760. The nearest match in the support set is "Juba is a city in Albania." That is correctly predicted as false; Juba is the capital of South Sudan and not a city in Albania.

# As one final thing to look at as an intro to the utility of Reexpress two (and SDM estimators, more generally), go back to Charts and examine the distribution of ood_eval.ood_random_shuffle.gemma_4_31b_it_4bit (which is a far out-of-distribution dataset) with that of the calibration data. (See https://raw.githubusercontent.com/ReexpressAI/Reexpress_two/main/documentation/tutorials/python_cli_to_app_tutorial/calibration_vs_far_ood.jpg) That is a clear example of the importance of geometry-aware calibration methods like SDM estimators.
